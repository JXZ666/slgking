...
| **设一次就够** | 左侧栏「更多工具…」收纳不常用的：标签译名 / 偏好权重 / 翻译设置 / 扫描本地目录；右上角齿轮 ⚙ 打开「设置」，主题和「关于」都在里面 |
...
6. **冻结的 exe 是个快照，不会读旁边的源码** —— 单文件模式下模块都解开在
   ...
   「设置 → 关于」里的 `v0.19.0 · exe · 2026-09-20 22:55` 就是给这个用的：
   时间没变，就是没重新打包。`--version` 打印同一行（侧栏只显示 `v0.19.0`）。
