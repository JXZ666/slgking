...
| **看得舒服** | 浅色 / 深色 / 跟随系统三种主题，在「设置」里切换；左侧「帮助文档」有使用说明和常见问题 |
| **设一次就够** | 左侧栏「更多工具…」收纳不常用的：检查更新 / 标签译名 / 偏好权重 / 翻译设置 / 扫描本地目录；右上角齿轮 ⚙ 打开「设置」，主题和「关于」都在里面 |
...
4. 左侧「**更多工具…**」→「**扫描本地目录…**」把游戏库目录（比如 `D:\Games\VisualNovels\`）
   里的游戏对上号，自动标「已下载」。
5. 左侧「**更多工具…**」→「**检查更新**」看哪些落后于站点。
...
界面是中文的，站点是英文的。左侧「**更多工具…**」→「**翻译设置…**」里选引擎：
...
`ENRICH_PER_SYNC` 从 400 降到 150，因为**一款游戏现在算两个请求**：详情页一个，
封面图一个，两个都走同一个 1 请求/秒的闸门。400 的话一次同步要跑十几分钟。

**同步自带封面。** 抓详情页的时候顺手把封面图下完，一款游戏下完才轮到下一款，
不用再另外点一次「下载封面」。中途按停止的话，同步停在**这一款之前**——
这款和它后面的都算没同步过，下次整款重来，不会留下半张图的孤儿行。
（老库里那些 `pending:` 标记的行还是得靠「下载封面」补，那是存量欠账。）
...
6. **冻结的 exe 是个快照，不会读旁边的源码** —— 单文件模式下模块都解开在
   ...
   「设置 → 关于」里的 `v0.21.0 · exe · 2026-09-22 01:15` 就是给这个用的：
   时间没变，就是没重新打包。`--version` 打印同一行（侧栏只显示 `v0.21.0`）。
